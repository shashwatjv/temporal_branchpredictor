...
void print_cache_stats(CACHE *stats)
{
    ...
    cout << stats->NAME;
    cout << " PREFETCH  FILL:   " << setw(10) << stats->STATS.pf_fill << "  USEFUL: " << setw(10) << stats->STATS.pf_useful << " USELESS: " << setw(10) << stats->STATS.pf_useless;
    cout << endl;

    cout << endl;
}

void print_drc_stats()
{
#ifdef DRAM_CACHE
    cout << endl;
    cout << "DRAM CACHE STATS" << endl;
    cout << "malloced drc_blocks: " << drc_blocks << " sizeof BLOCK: " << sizeof(BLOCK) << endl;
    cout << "num_valid fill: " << uncore.DRC.num_valid << endl;
    cout << "num_zero_reuse: " << uncore.DRC.num_zero_reuse << endl;
#endif
}
...
void print_dram_stats()
{
    ...
    cout << " RQ ROW_BUFFER_HIT: " << setw(10) << uncore.DRAM.RQ.ROW_BUFFER_HIT << "  ROW_BUFFER_MISS: " << setw(10) << uncore.DRAM.RQ.ROW_BUFFER_MISS << endl;
    ...
    for (uint32_t i=0; i<NUM_TYPES+1; i++) {
        for (uint32_t j=0; j<NUM_TYPES+1; j++)
            cout << " DBUS_OCCUPIED_BY_" << i << " BLOCKING_" << j << " numbers: " << uncore.DRAM.dbus_congested[i][j] << endl;
        cout << endl;
    }
...
void print_deadlock(uint32_t i)
{
    ...
    // print DRC
    cout << "Last DRC read mode: " << last_drc_read_mode << " write mode: " << last_drc_write_mode << endl;
...
int main(int argc, char** argv)
{
    ...
    // default: 4 = (64 / 16) * (3200 / 3200)
    DRC_TAG_RETURN_TIME = ((DRC_TAG_SIZE/DRC_CHANNEL_WIDTH)*CPU_FREQ)/DRC_MTPS;
    DRC_DATA_RETURN_TIME = ((DRC_BLOCK_SIZE/DRC_CHANNEL_WIDTH)*CPU_FREQ)/DRC_MTPS;

#ifdef DRAM_CACHE
    printf("On-chip DRAM$ Channels: %u Width: %u-bit Data Rate: %u MT/s\n",
            DRC_CHANNELS, 8*DRC_CHANNEL_WIDTH, DRC_MTPS);
#endif
...
#ifdef DRAM_CACHE
        uncore.LLC.lower_level = &uncore.DRC;

        // ON-PACKAGE DRAM CACHE
        uncore.DRC.fill_level = FILL_DRC;
        uncore.DRC.upper_level_icache[i] = &uncore.LLC;
        uncore.DRC.upper_level_dcache[i] = &uncore.LLC;
        uncore.DRC.lower_level = &uncore.DRAM;
        uncore.DRC.RQ.is_RQ = 1;
        uncore.DRC.WQ.is_WQ = 1;
#endif
...
#ifdef DRAM_CACHE
        uncore.DRAM.upper_level_icache[i] = &uncore.DRC;
        uncore.DRAM.upper_level_dcache[i] = &uncore.DRC;
#endif
    ...
    last_drc_read_mode = 0;
    last_drc_write_mode = 0;
    drc_blocks = 0;
